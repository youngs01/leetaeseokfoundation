...
export default function LectureDetailPage({ params }: { params: { id: string } }) {
  ...
  const lecture = getContentById<LectureItem>("lecture", params.id)
  ...
  const relatedLectures = getRelatedContent<LectureItem>("lecture", params.id, 2)

  // Get previous and next lectures
  const { prev, next } = getPrevNextContent<LectureItem>("lecture", params.id)
