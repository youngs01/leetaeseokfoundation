...
export default function VideoDetailPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    try {
      // Get the video item
      const videoData = getContentById<VideoItem>("video", params.id)
      ...
      // Get related videos
      const related = getRelatedContent<VideoItem>("video", params.id, 3)
  ...
  if (error || !video) {
    return (
      <div className="container mx-auto px-4 py-12 text-center">
        ...
        <p className="text-muted-foreground mb-6">{error || "영상을 찾을 수 없습니다."}</p>
        <Button asChild>
          <Link href="/news/videos">영상 목록으로 돌아가기</Link>
        </Button>
      </div>
    )
  }
